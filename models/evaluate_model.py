...
def evaluate(model, test_loader, model_name, rel=None, save_path=""):
    """
    评估函数：
      - 可选：对响应序列 data["rseqs"] 做随机翻转（10% 概率），可视需要保留或删除。
      - 针对不同 model_name，在循环中调用相应模型的 forward/predict，并将结果存到 y。
      - 最终对 y 与真实标签进行 masked_select 后，计算 AUC 和 ACC。
    """
    # （可选）数据增强：随机翻转响应
    for data in test_loader:
        if random.random() < 0.1:  # 10% 概率翻转
            data["rseqs"] = 1 - data["rseqs"]
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            # 下面只是示例，对于 dimkt、qseqs, cseqs... 你可以按需保留
            if model_name in ["dimkt"]:
                ...
                sd, qd = sd.to(device), qd.to(device)
                sdshft, qdshft = sdshft.to(device), qdshft.to(device)
            else:
                q, c, r = dcur["qseqs"], dcur["cseqs"], dcur["rseqs"]
                qshft, cshft, rshft = dcur["shft_qseqs"], dcur["shft_cseqs"], dcur["shft_rseqs"]
                q, c, r = q.to(device), c.to(device), r.to(device)
            ...
            if model.model_name in que_type_models and model_name not in ["lpkt", "rkt"]:
                model.model.eval()  # 某些模型内部包含了 .model
            else:
                model.eval()
            ...
            if model_name == "atdkt":
            ...
            elif model_name in ["akt", "folibikt", "akt_vector", "akt_norasch",
                                "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos",
                ...
                # 如果都不匹配，说明没有对这个 model_name 做处理
                raise ValueError(f"Unsupported model_name: {model_name}")

            # 若 save_path 不为空，可以保存预测结果
            if save_path != "":
                # 需要你自行实现 save_cur_predict_result
                # or 将其注释掉
            ...
            # 如果 y 仍然是 None，就会报错
            if y is None:
                raise RuntimeError(f"Model forward returned None for model_name={model_name}.")
...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn","skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt", "folibikt","dtransformer","simplekt", "KeenKT", "bakt_time", "sparsekt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "akt", "folibikt", "dtransformer", "simplekt", "KeenKT","bakt_time", "sparsekt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
...
def group_fusion(dmerge, model, model_name, fusion_type, fout):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "dtransformer", "akt","folibikt","simplekt","KeenKT", "bakt_time", "sparsekt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
...
def predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    for k in range(t, end):
        ...
        if model_name in ["atdkt"]: ## need change!
           ...
           pred = y[0][-1]
        elif model_name in ["simplekt","KeenKT", "sparsekt"]:
           if qout != None:
               curq = torch.tensor([[qout.item()]]).to(device)
               qinshft = torch.cat((qin[:,1:], curq), axis=1)
           else:
